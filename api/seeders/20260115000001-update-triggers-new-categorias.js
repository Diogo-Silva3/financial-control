...
module.exports = {
	async up (queryInterface, Sequelize) {
		...
		await sequelize.query(`DROP TRIGGER IF EXISTS ${database}.despesas_AFTER_INSERT;`);
		
		// Criar trigger atualizado com novas categorias
		return sequelize.query(
			`CREATE DEFINER = CURRENT_USER TRIGGER ${database}.despesas_AFTER_INSERT AFTER INSERT ON despesas FOR EACH ROW ` +
			`BEGIN ` +
			...
			`WHEN @categoria = 8 THEN UPDATE ${database}.relatorios SET outros = @somaCategoria WHERE ano = @ano and mes = @mes and usuario_id = @usuario; ` +
			`WHEN @categoria = 9 THEN UPDATE ${database}.relatorios SET internet = @somaCategoria WHERE ano = @ano and mes = @mes and usuario_id = @usuario; ` +
			`WHEN @categoria = 10 THEN UPDATE ${database}.relatorios SET neoenergia = @somaCategoria WHERE ano = @ano and mes = @mes and usuario_id = @usuario; ` +
			`WHEN @categoria = 11 THEN UPDATE ${database}.relatorios SET compesa = @somaCategoria WHERE ano = @ano and mes = @mes and usuario_id = @usuario; ` +
			`END CASE; ` +
			`END;`
    	)
	},
	...
	async down (queryInterface, Sequelize) {
    	const sequelize = new Sequelize("", username, password, {
      	...
      	return sequelize.query(`DROP TRIGGER IF EXISTS ${database}.despesas_AFTER_INSERT;`);
    },
};
